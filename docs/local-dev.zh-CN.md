...
- `wrangler` 已通过你的 Cloudflare 账户认证 (每台机器执行一次
  ...
  Workers 模拟器 (通过 `@opennextjs/cloudflare`)。
...
这条命令先跑 `opennextjs-cloudflare build` 把 Next.js 产物打成 Workers
bundle (输出到 `.open-next/`),然后用 `wrangler dev` 监听 8787 端口。
打开 http://localhost:8787,用第 5 步种入的凭据登录。第一次登录会要求
...
`http://localhost:8787` 加进桶的 CORS `AllowedOrigins`。完整规则和
