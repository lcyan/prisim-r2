...
- 新增 `stores/theme.ts`（Zustand + `persist` middleware）：state `theme: 'blue' | 'orange' | 'green'`；action `setTheme(t)`；localStorage key=`prisim-r2-theme`，默认 `'blue'`。
- 切换时 `setTheme` 同步：写 store + 写 localStorage + `document.documentElement.dataset.theme = t`。
- **FOUC 防护**：`app/layout.tsx` 的 `<head>` 注入 `<Script id="theme-init" strategy="beforeInteractive">`：

  ```js
  (function () {
    try {
      var raw = localStorage.getItem('prisim-r2-theme');
      var t = raw ? (JSON.parse(raw).state || {}).theme : null;
      if (t !== 'blue' && t !== 'orange' && t !== 'green') t = 'blue';
  ...
  与 Zustand persist v4 的存储结构 `{ "state": { "theme": "..." }, "version": 0 }` 对齐。
...
- 切 bucket：写 Zustand `useActiveBucket` store + `router.push('/buckets/' + name)`
...
- `stores/theme.ts`
- `stores/active-bucket.ts`（仓库目前只有 `active-connection.ts` 与 `selected-keys.ts` / `upload-queue.ts`，没有 active-bucket）
- `components/features/dashboard/theme-switcher.tsx`（popover 用 shadcn 的 `DropdownMenu` 或 `Popover` —— 实施时择一，spec 不强制）
...
- `tests/unit/stores/theme.test.ts` — `setTheme` 写 localStorage、初始值 fallback `blue`、setTheme 后 `document.documentElement.dataset.theme` 同步
- `tests/unit/dashboard/pick-home-route.test.ts` — 验证新逻辑（dashboard-first 除非 callbackUrl）
- `tests/unit/features/dashboard/theme-switcher.test.tsx` — 渲染三行、点击触发 setTheme（@testing-library/react；若 jsdom 不在 vitest 配置中，先添加）
...
| FOUC：刷新瞬间 `<html>` 没 data-theme，闪默认主题 | 在 `<head>` 注入 `<Script strategy="beforeInteractive">` 同步读 localStorage |
...
| FOUC script 解析 Zustand persist 格式失败 | 严格匹配 v4 格式 `{ "state": { "theme": "blue" }, "version": 0 }`；用 `?.state?.theme` 链式访问 |
| Vitest 缺少 jsdom 环境跑 React 组件测试 | 添加 `vitest.config.ts` 的 `test.environment = 'jsdom'` 或仅对该测试文件加 `// @vitest-environment jsdom` 注释 |
